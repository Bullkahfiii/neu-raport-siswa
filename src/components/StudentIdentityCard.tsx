...
export function StudentIdentityCard({ student }: StudentIdentityCardProps) {
  return (
    <div className="bg-card rounded-xl shadow-card p-6 animate-fade-in">
      ...
      {(() => {
        const hasPilihan1 = student.pilihanPTNPertama && student.pilihanPTNPertama !== '-';
        const hasPilihan2 = student.pilihanPTNKedua && student.pilihanPTNKedua !== '-';
        if (!hasPilihan1 && !hasPilihan2) return null;
        const bothFilled = hasPilihan1 && hasPilihan2;
        return (
          <div className="mt-6 p-4 bg-gradient-to-r from-primary/10 to-accent/10 rounded-lg">
            ...
            <div className={`grid grid-cols-1 ${bothFilled ? 'md:grid-cols-2' : ''} gap-3`}>
              {hasPilihan1 && (
